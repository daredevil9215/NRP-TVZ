...
def slice_image(image : np.ndarray):

    res = []
    z = int(np.floor(image.shape[1] / 7))
    t = image.shape[1] // z
    ...
    for k in range(t):
        ...
        for i in range(image.shape[0]):
            ...
            for j in range(k * z, (k + 1) * z):

                temp.append(image[i][j])

            im1.append(temp)

        im1 = cv2.resize(np.array(im1), (32, 32))
        im1 = (im1.reshape(1, -1).astype(np.float32) - 127.5) / 127.5
        im1 = im1[0]
